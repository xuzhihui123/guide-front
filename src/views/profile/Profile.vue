  ...
  <div id="profile">
    <div class="profile-top_bg">
      ...
      <div class="profile-top_login_view">
        ...
        <div class="submit-login" @click="toLogin" v-show="topSubmitLogin">点击登录</div>
        <div class="profile-top_login_view_detail" v-show="topProfileView">
          <div class="top">
            <span class="nick_name">{{singleUserInfo.user_nick}}</span>
            <p class="nick_name_profile" @click="goProfileDetail">
              <span>编辑资料</span>
...
export default {
  ...
  methods: {
    ...
    // 获取localstorage的用户信息
    getUserInfo () {
      this.singleUserInfo = JSON.parse(
    ...
    finish (type) {
      if (type === 'blob') {
        ...
        this.$refs.cropper.getCropData(data => {
          // 转换file对象 上传服务器
          const TrueFile = baseTofile.dataURLtoFile(data, 'avator')
                    ...
                    if (res.code === '200') {
                      ...
                      getUserInfoById({
                        ...
                        if (resData.status.code === '200') {
                          // 获取新的用户信息
                          const { user_avatar: change_user_acator } = resData.data
